                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    for (int j = 0; j < benCount; j++)
                    {
                        ...
                        var ben = new AssistanceBeneficiary
                        {
                            ...
                            FileNumber = "FILE-" + _random.Next(1000, 9999).ToString(),
                            FamilyName = _lastNames[_random.Next(_lastNames.Length)],
                            City = _random.Next(0, 2) == 0 ? "غزة" : "خان يونس",
                            SectorId = sector.Id,
                            FamilyCount = _random.Next(1, 10),
